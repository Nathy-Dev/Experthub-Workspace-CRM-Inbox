...
// API_BASE chooses local dev server when opened locally (file:// or localhost),
// otherwise defaults to the production host.
const DEFAULT_REMOTE = 'https://experthub-workspace-crm-inbox.onrender.com';
...
if (location.protocol === 'file:' || location.hostname === 'localhost' || location.hostname === '127.0.0.1' || location.hostname === '') {
    ...
    // Use same origin when the page is served from a host
    API_BASE = `${location.protocol}//${location.hostname}${location.port ? ':' + location.port : ''}`;
} else {
    API_BASE = DEFAULT_REMOTE;
}
...
try {
    // Build a WebSocket URL based on API_BASE (handles localhost, served origins, and remote)
    let wsUrl;
    try {
        const parsed = new URL(API_BASE);
        wsUrl = `${parsed.protocol === 'https:' ? 'wss:' : 'ws:'}//${parsed.hostname}${parsed.port ? ':' + parsed.port : ''}`;
    } catch (e) {
        // API_BASE may be a relative path; fall back to current host or localhost
        const host = location.hostname && location.hostname !== '' ? location.hostname : 'localhost';
        wsUrl = `${location.protocol === 'https:' ? 'wss:' : 'ws:'}//${host}:3000`;
    }
